...
class MinistriesRepository:
    ...
    async def list_ministries(self, parish_id: uuid.UUID | None = None) -> list[Ministry]:
        stmt = select(Ministry).where(Ministry.is_deleted.is_(False))
        if parish_id is not None:
            stmt = stmt.where(Ministry.parish_id == parish_id)
        stmt = stmt.order_by(Ministry.name)
