...
class Family(Base, UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin):
    ...
    centrale_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("centrales.id"), nullable=True
    )
    scc_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("small_christian_communities.id"), nullable=True
    )
