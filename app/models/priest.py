...
class Priest(Base, UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin):
    ...
    title: Mapped[str] = mapped_column(
        String(50), default="Padiri", nullable=False
    )  # Monseigneur, Padiri, Diakoni...
    ...
    congregation: Mapped[str | None] = mapped_column(
        String(200), nullable=True
    )  # E.g. Salesians, White Fathers
    ...
    # Current assignment
    current_parish_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("parishes.id"), nullable=True
    )
    current_role: Mapped[str | None] = mapped_column(
        String(100), nullable=True
    )  # Curé, Vicaire, Aumônier, etc.

    assignments: Mapped[list["ClergyAssignment"]] = relationship(
        "ClergyAssignment", back_populates="priest"
    )
...
class ClergyAssignment(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    priest_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("priests.id"), nullable=False
    )
